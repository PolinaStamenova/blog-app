...
RSpec.describe 'Post view', type: :feature do
  ...
  describe 'Post show' do
    ...
    let(:user_two) { User.create(name: 'Marina', email: 'marina@example.com', password: 'password') }
    ...
    describe 'display post information' do
      ...
      before do
        Comment.create(text: 'Comment from Marina', user: user_two, post: post)
        Comment.create(text: 'Comment from Polina', user: user, post: post)
      ...
      it 'display the comment each commentor left.' do
        expect(page).to have_content('Comment from Polina')
        expect(page).to have_content('Comment from Marina')
      end
    end
  end
end
